...
### 成果
1. 今天开发相当顺利, 本地/github测例全部通过! 坚持这么多天实属不易, 撒花~ ✨✨✨
...
### 成果
1. 一头雾水, 自己在文件系统这块积累的太少了, 对于xv6, linux内核的文件系统都有一点点了解, 但又都是浅尝辄止, 这次实验又是用的新的easy-fs, 所以大脑里的知识混乱无比...
2. 最近进度太慢了, 明天务必要全神贯注, 全力推进!

## day 44
### 计划
1. 搞清楚昨天的bug.
2. 将 easy-fs 代码完全合入 os6. 

### 成果
1. 昨天遇到的问题, 原因在于没有将 VirtIO 总线的 MMIO 物理地址区间 (从 0x10001000 开头的 4KiB) 加入内核页表. 
2. 已经将 easy-fs 的代码合入 os6, 并且将 app loader 函数的实现修改为基于读取磁盘文件, 能顺利运行.
...
### 成果
1. 终于看明白 easy-fs 的代码在哪了, 我一直以为会在 os6 目录里, 找了半天都没找到. 但是其实是在os6的同级目录下, 然后在Cargo.toml 里自定义路径. 
2. 对着 os-ref, 把 os6/drivers 的代码报错都改好了. 
3. 初始化 ROOT_INODE 时又出现类似爆栈的错误, 明天深入调试一下.

## day 37--41
### 计划
1. 这几天各种杂事有点多, 而且要考英语, 精力分散, 生产力不高.

### 成果
1. 英语考完了, 撒花~
2. 重构 syscall.rs, 将各种函数按照功能分类到不同文件. 
3. 按照文档, 将 sys_read 和 sys_write 重构为基于 File 的实现.
